...
% state machine to the port array module to open and close valves, and set LED brightness. 
...
% so the following line is triggering the second valve (valve is ethernet
% port)
...
if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
    S.GUI.WaterValveTime = 1;	  % in sec SET-UP SPECIFIC #reward size
    S.GUI.AutoWaterValveTime = 1;  % in sec SET-UP SPECIFIC #time for givedrop sma
    ...
    S.GUI.RewardChangeFactor  = 3; % change in reward time compared to regular WaterValveTime % if the flage is 2 (large reward due to comment above)- the reward size is multiplied by this
    S.GUI.RewardOmissionProb = 0.2; % Probability of a complete reward omission % if the rand is < than this, the flag is 0 and the mouse gets small reward
%     S.GUI.AnswerPeriodFirstLick = 10;	% in sec #not being used
    ...
    S.GUI.InterTrialInterval = 2;	  % in sec #time for tup on reward consumption, not licking and then until trial end
    ...
    S.GUI.Lx_motor_pos = 210000;       %210000 [0 1000000];
    S.GUI.Ly_motor_pos = 420000;       %420000 [0 1000000];
...
current_trial_num_in_block_seq=1;
...
for currentTrial = 1:MaxTrials
    ...
    sma = NewStateMachine;
    sma = SetGlobalTimer(sma, 'TimerID', 1, 'Duration', 0.004, 'OnsetDelay', 0,...
                         'Channel', 'BNC1', 'OnLevel', 1, 'OffLevel', 0,...
                         'Loop', 1, 'SendGlobalTimerEvents', 0, 'LoopInterval', 0.004); 
    ...
    switch S.GUI.ProtocolType %switch 
...
%                 sma = AddState(sma, 'Name', 'OffStatePreSample', 'Timer', 10, 'StateChangeConditions', {'Tup','GiveDro2'},'OutputActions', []); %MoveLickPortIn
%                 sma = AddState(sma, 'Name', 'GiveDro', 'Timer', 10,'StateChangeConditions', {'Tup', 'AnswerPeriodAutoWater'},'OutputActions',{'Valve1', 1, 'PWM1', 1});%instead of 5 put S.GUI.AutoWaterValveTime % turn on water
%                 sma = AddState(sma, 'Name', 'AnswerPeriodAutoWater', 'Timer', 10,'StateChangeConditions', {'Tup', 'exit'},'OutputActions', {'GlobalTimerCancel', 1});%instead of 5 put S.GUI.AutoWaterValveTime % turn on water
%                 sma = AddState(sma, 'Name', 'GiveDro2', 'Timer', 10,'StateChangeConditions', {'Port1In','GiveDro', 'Tup', 'exit' },'OutputActions', []);%,'Tup', 'OffStatePreSample'instead of 5 put S.GUI.AutoWaterValveTime % turn on water
% %                 sma = AddState(sma, 'Name', 'LickDetector', 'Timer', 5,'StateChangeConditions', {'Tup', 'OffStatePreSample'},'OutputActions', []);%instead of 5 put S.GUI.AutoWaterValveTime % turn on water
% 
...
%             end
            ...
            sma = AddState(sma, 'Name', 'AnswerPeriod', 'Timer',10,'StateChangeConditions', {'Port1In', 'Reward', 'Tup', 'NoResponse'},'OutputActions', MoveLickPortIn);%output needs to be MoveLickPortIn % advance lickport and wait for response
            ...
            sma = AddState(sma, 'Name', 'AnswerPeriodAutoWater', 'Timer', 20,'StateChangeConditions', {'Port1In', 'RewardConsumption', 'Tup', 'NoResponse'},'OutputActions', MoveLickPortIn);%output needs to be MoveLickPortIn % advance lickport and wait for response
            ...
            sma = AddState(sma, 'Name', 'RewardConsumption', 'Timer', 10,'StateChangeConditions', {'Tup', 'InterTrialInterval'},'OutputActions', []); % reward consumption
            ...
            sma = AddState(sma, 'Name', 'NoResponse', 'Timer', 10, 'StateChangeConditions', {'Tup', 'InterTrialInterval'},'OutputActions',[]); % no response - wait same time as for reward consumption, as a time out

            sma = AddState(sma, 'Name', 'InterTrialInterval', 'Timer', S.GUI.InterTrialInterval,'StateChangeConditions', {'Tup', 'TrialEnd'},'OutputActions', MoveLickPortOut); %output needs to be MoveLickPortOut % retract lickport
