...
motors_properties.Ly_motor_num = 6;
...
%     S.GUI.AnswerPeriodFirstLick = 10;	% in sec #not being used
    S.GUI.NumLicksForReward =1.5;% changed it from 2. its meaning- if larger than 1, mouse gets reward only after xxx trials, otherwize they get reward from first lick
...
for currentTrial = 1:MaxTrials
    ...
    sma = SetGlobalTimer(sma, 'TimerID', 1, 'Duration', 0.5, 'OnsetDelay', 0,...
                         ...
                         'Loop', 1, 'SendGlobalTimerEvents', 0, 'LoopInterval', 0.5); 
...
%             end
            ...
            sma = AddState(sma, 'Name', 'AnswerPeriod', 'Timer',100,'StateChangeConditions', {'Port1In', 'Reward', 'Tup', 'NoResponse'},'OutputActions', []);%output needs to be MoveLickPortIn % advance lickport and wait for response
            ...
            sma = AddState(sma, 'Name', 'AnswerPeriodAutoWater', 'Timer', 5,'StateChangeConditions', {'Port1In', 'RewardConsumption', 'Tup', 'NoResponse'},'OutputActions', []);%output needs to be MoveLickPortIn % advance lickport and wait for response
            ...
            sma = AddState(sma, 'Name', 'RewardConsumption', 'Timer', 100,'StateChangeConditions', {'Tup', 'InterTrialInterval'},'OutputActions', []); % reward consumption
            ...
            sma = AddState(sma, 'Name', 'InterTrialInterval', 'Timer', S.GUI.InterTrialInterval,'StateChangeConditions', {'Tup', 'TrialEnd'},'OutputActions', []); %output needs to be MoveLickPortOut % retract lickport
